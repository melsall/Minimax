...
public class NimRunner {
    ...
    public static ArrayList <Integer> makeRandomStates(){
        ...
        for(int i = 0; i < (int)(Math.random()*15) + 1; i++){
            states.add((int) (Math.random() * 15 ) + 1); // 
        }
        return states;
    }
...
//         }
//         else{
//             ArrayList <ArrayList <Integer>> moves = new ArrayList <> (); // make a moves array to hold the list of all the possible moves
//             for (int p = 0; p < states.size(); p++){ // iterate through each pile
//                 int [][] currentMove = new int [states.size];
//                 for (int c = 1; c < states.get(p); c++){ // iterate through each object in the piles
//                     currentMove [r] = c; // making array of the possible moves, taking c objects each time
//                 }
//                 moves.add(Arrays.asList(currentMove)); // adding the possible moves for a given pile to the moves list
//             }
//             // evaluating how "good" all of the moves are
